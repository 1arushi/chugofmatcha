    ...
    <div style={{ ...styles.screen, background: C.bg, padding: "0 0 0", position: "relative" }}>
      ...
      <div style={{ padding: "0 28px 0", display: tab === "cafe" ? "block" : "none" }}>
        ...
        <div style={{ marginBottom: 20, position: "relative" }}>
          ...
          />
          {cafeSearchOpen && (() => {
            // Find matching cafes from community logs
            const allCafeNames = communityStats ? Object.keys(communityStats.cafeVisits).filter(c => !communityStats.homemadeCafes || !communityStats.homemadeCafes.includes(c)) : [...new Set(logs.map(l => l.cafe))];
              ...
              <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, background: C.cardLight, borderRadius: 16, overflow: "hidden", zIndex: 20, boxShadow: "0 4px 16px rgba(0,0,0,0.2)" }}>
                {matches.map(cafeName => (
                  <div key={cafeName} onClick={async () => {
                    ...
                    // Load full stats for this cafe from community logs
                    try {
        ...
        {/* Cafe search result */}
        {cafeSearchResult && (
          <div style={{ background: C.card, borderRadius: 20, padding: "18px 20px", marginBottom: 20 }}>
